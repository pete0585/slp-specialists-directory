...
export default async function sitemap(): Promise<MetadataRoute.Sitemap> {
  ...
  const listingPages = (listings ?? []).map(l => ({ url: `${siteUrl}/listings/${l.slug}`, lastModified: new Date(l.updated_at ?? Date.now()), changeFrequency: 'monthly' as const, priority: 0.7 }))
