...
export default async function CityPage() {
  ...
  const faqLd = {
    ...
    mainEntity: [
      {
        "@type": "Question",
        name: "How many speech-language pathologist are in Memphis, TN?",
        acceptedAnswer: {
          "@type": "Answer",
          text: "Speech-Language Pathologist Directory lists 35+ speech-language pathologist in Memphis, Tennessee. Counts change as new listings are seeded.",
        },
      },
      {
        "@type": "Question",
        name: "How do I find speech-language pathologist in Memphis?",
        acceptedAnswer: {
          "@type": "Answer",
          text: "Search findslpspecialist.com and filter by Memphis. Compare listed providers, then contact the one that fits.",
        },
      },
    ],
  }
  return (
    <>
      ...
      <main className="mx-auto max-w-5xl px-4 py-10">
        <p className="text-sm text-neutral-500">Memphis, TN</p>
        <h1 className="mt-2 text-3xl font-bold tracking-tight">
          Speech-Language Pathologist in Memphis, TN
        </h1>
        <p className="mt-3 max-w-2xl text-neutral-600">
          35+ listed speech-language pathologist in the Memphis area. Pages are generated from live directory listings — not outreach.
        </p>
          ...
          {listings.map((row: Record<string, unknown>, i: number) => (
            <li key={String(row.id || row.slug || i)} className="rounded-xl border border-neutral-200 p-4">
              ...
              <p className="mt-1 text-sm text-neutral-500">
                {String(row.city || "Memphis")}, {String(row.state || "TN")}
              </p>
            </li>
          ))}
        </ul>
