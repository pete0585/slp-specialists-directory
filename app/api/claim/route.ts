...
export async function POST(request: NextRequest) {
  try {
    ...
    const { data: listing, error: listingError } = await supabase.from('slp_listings').select('id, full_name, email, is_claimed').eq('id', listingId).single()
    if (listingError || !listing) return NextResponse.json({ error: 'Listing not found' }, { status: 404 })
    if (listing.is_claimed) return NextResponse.json({ error: 'This listing has already been claimed' }, { status: 400 })
    ...
    const displayName = listing.full_name || 'Your listing'
